...
/* ═══════════════════════════════════════════════════════ SAMPLE MODE (W-014)
   THE DEMONSTRATION CASE. Every number below is fabricated: it belongs to no
   patient, it was not measured on any scanner, and it is not a cut-off, a
   calibration constant or a coefficient — so CLAUDE.md § 1 does not reach it,
   and nothing in the data layer sources it. What DOES reach it is § 1.2's rule
   about silence: a fabricated case that does not say it is fabricated is the
   hazard, which is why the mode is carried by a watermark on every printed
   sheet, a sentence in both footers, and locked sheets.

   It lives here rather than in v2/data/ deliberately. The data layer holds
   published evidence; a demonstration patient is view furniture, and filing it
   beside the cut-offs would put an invented record in the one folder whose
   entire contract is that nothing in it is invented. */
const SAMPLE_CASE = {
  path: 'ge', fieldStrength: '1.5T', cohort: 'adult', scope: 'cleared',
  indication: 'steatotic-liver-disease',
  accession: 'SAMPLE-0001', studyDate: '2026-01-15', age: 54,
  values: {pdff: 14.2, r2star: 62, mre: 3.1, t1: 820, ct1: 795, adc: 1.15,
           ast: 48, alt: 61, plt: 178, ferritin: 320, tsat: 34,
           bmi: 31.2, ascites: false, altUln: 33, ggt: 88}
};
...
/* STATELESS BY DECISION. The mode is never written to sessionStorage: a reload
   ...
   looking like a sample. */
let viewMode = 'sample';
let selection = applySelection(createSelection(), SAMPLE_CASE);

function enterSample() {
  viewMode = 'sample';
  selection = applySelection(createSelection(), SAMPLE_CASE);
  removed = {};
...
/* Tab moves along the ROUTE, not along the DOM. Off either end of the route,
   and anywhere outside it — the sample control, the tier radios, the printing
   switches — the browser's own behaviour is left alone. */
...
function buildModel(report, profile, sel) {
  ...
  return {report: report,
          ...
          composite: buildComposite(report, labs, reliability),
...
function wireSelectionScreen() {
  ...
  app.querySelectorAll('input[name="scope"]').forEach(el =>
    el.addEventListener('change', () => {
      selection = applySelection(selection, {scope: el.value});
      reRender();
    }));
