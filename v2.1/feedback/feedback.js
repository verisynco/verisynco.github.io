...
/* W-168 moves this 1.5 -> 1.6: a submission from this line now carries which
   line it came from. The tag rides the existing `versions` string as its first
   segment (no new Tally field, so nothing to configure on form XxPG6Y); frozen
   V2 sends no tag, and an untagged submission is a V2 one. */
const FEEDBACK_VERSION = '1.7';

/* Which published line this feedback file belongs to. V2.1's own copy says
   'v2.1'; frozen V2's copy carries no such constant. */
const LINE = 'v2.1';
...
function buildPayload(ctx) {
  ...
  return {
    ...
    /* W-168: the published line is the first segment, ahead of the four version
       stamps — "v2.1/<app>/<thresholds>/<cutoffs>/<disclaimer>". */
    versions:      [LINE, v.app, v.thresholds, v.cutoffs, v.disclaimer].join('/'),
...
/* W-173 — THE EMBED DIALECT, MEASURED BEFORE IT WAS ADOPTED. `tally.so/r/<id>`
   is the form's own full PAGE, and framing a page inside a 28rem panel is what
   put a 32px title and 182px of empty space above the first question. Read back
   from a real headless browser on 2026-09-07, both dialects at 440px wide:

     /r/<id>                     title rendered at 32px, first question 182px down
     /embed/<id>?hideTitle=1     title present but NOT rendered, first question 29px down

   What it does NOT buy is a smaller form: the questions measure 18px and the
   options 16px in BOTH, because that scale is set in Tally and no stylesheet
   here can reach into another origin. The panel's own type moved up to meet it
   instead (feedback.css, `G20`).

   The hidden fields are unaffected -- the same probe showed both dialects
   parsing the query into the identical page state, and the twelve columns were
   verified by a real submission on 2026-08-25. `transparentBackground` is asked
   for explicitly so the paper ground under the frame is ours (feedback.css
   `.fb-frame`), and `dynamicHeight` is deliberately NOT asked for: nothing here
   listens for the resize message it would send, and the panel's height is the
   window's to decide. */
const EMBED_OPTS = 'hideTitle=1&alignLeft=1&transparentBackground=1';

function formUrl(payload) {
  return 'https://tally.so/embed/' + TALLY.formId + '?' + EMBED_OPTS +
         '&' + payloadToQuery(payload);
}
...
function panelHtml(state) {
  ...
  return '<div class="fb-root">' + tab +
    ...
    ' data-fb-yield="false">' +
      ...
      /* W-173 round 2. The frame is wrapped because the only way to shrink a
         third party's type from here is to scale the whole frame: the wrapper
         is the box the panel lays out, and the frame inside it is drawn at
         twice that size and scaled to half. A transform on the frame alone
         would leave its layout box full-size and overflow the panel. */
      '<div class="fb-framewrap">' +
