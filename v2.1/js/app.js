...
const V21_APP_VERSION = '0.16.0';  /* W-167 moves this 0.15.0 -> 0.16.0: the
   ...
   `toolbar(...)` in both render branches. In memory only (Option A, developer
   decision 2026-09-07) — no storage, so the terms text and
   V21_DISCLAIMER_VERSION are untouched. No network, no clinical value, no hash
   lock.
   W-161 moved this 0.14.6 -> 0.14.7: the
   ...
   V21_DISCLAIMER_VERSION goes 1.1 -> 1.2 and this released-change counter moves
   ...
   request that W-009 removed — so V21_DISCLAIMER_VERSION goes 1.0 -> 1.1 and this
   ...
   toolbar button). readTheme/writeTheme/applyTheme/toggleTheme + V21_THEME_KEY
...
const V21_DISCLAIMER_VERSION = '1.2';
const V21_DISCLAIMER_KEY = 'veriliv-v21-disclaimer';
...
const V21_THEME_KEY = 'veriliv-v21-theme';

function readTheme() {
  try {
    const v = localStorage.getItem(V21_THEME_KEY);
    return v === 'dark' ? 'dark' : 'light';
  } catch (e) { return 'light'; }
}
function writeTheme(theme) {
  try { localStorage.setItem(V21_THEME_KEY, theme); } catch (e) {}
}
...
/* W-164. WHICH METHODOLOGY BLOCKS THE READER HAS OPENED. A screen-only concern,
   owned here and threaded down as a plain argument exactly the way bmiPopupOpen
   already is — never part of `selection`. `selection` is the clinical model and
   what the hash locks cover; a reading preference has no business moving a lock,
   and the sheet is rebuilt in one DOM write on every change, so a UI flag placed
   there would be exercised by every hash test for no reason.

   NOT PERSISTED, DELIBERATELY. A reload opens the blocks closed again. Carrying
   the open state across a reload would undo, on the reader's second visit, the
   whole point of the change — and the printed sheet is unaffected either way. */
let methodOpen = {};
function toggleMethodBlock(key) {
...
/* W-167. WHETHER THE READER HAS DISMISSED THE INTRODUCTION STRIP. Same shape and
   ...
   IN MEMORY ONLY (Option A, developer decision 2026-09-07). No sessionStorage,
   no localStorage — a reload shows the strip again, and the terms text (which
   enumerates exactly what is stored) does not have to move. The strip only ever
   draws in SAMPLE mode, so leaving the sample makes it irrelevant; switching
   scenario keeps a dismissal, which is the reader's stated intent. */
...
function readAck() {
  try {
    const raw = sessionStorage.getItem(V21_DISCLAIMER_KEY);
    if (!raw) return null;
    const rec = JSON.parse(raw);
    return (rec && rec.version === V21_DISCLAIMER_VERSION && rec.ts) ? rec : null;
  } catch (e) { return null; }
}
function writeAck(rec) {
  try { sessionStorage.setItem(V21_DISCLAIMER_KEY, JSON.stringify(rec)); } catch (e) {}
}

function acceptDisclaimer() {
  const rec = readAck() || {version: V21_DISCLAIMER_VERSION, ts: new Date().toISOString()};
  writeAck(rec);
  ...
  if (!report.ready) {
    ...
    app.innerHTML = toolbar({mode: viewMode}, true, sampleMenuAllowed(), sampleScenarioList(), sampleKey) +
      ...
      renderReport(model, profile, selection,
                   {app: V21_APP_VERSION,
                    disclaimer: V21_DISCLAIMER_VERSION,
...
function wireSelectionScreen() {
  ...
  /* W-167. The introduction strip's dismiss control. The strip's primary button
     carries `data-action="new-report"` and is bound by the line above; only the
     dismiss needs its own handler, which just flips the in-memory flag. Drawn
     only in SAMPLE mode and only while `introDismissed` is false (render.js
     `introStrip()`), so a listener on an absent element is never called. */
  app.querySelectorAll('button[data-action="dismiss-intro"]').forEach(el =>
  ...
  app.querySelectorAll('button[data-action="send-requestor"]').forEach(el =>
    el.addEventListener('click', () => {
      ...
      const mail = buildRequestorEmail(
        ...
        selection,
        {app: V21_APP_VERSION, disclaimer: V21_DISCLAIMER_VERSION, ackTs: window.__ackTs});
  ...
  /* W-164. The methodology sheet's collapse controls. The flag is this file's,
     the drawing is render.js's, and nothing about `selection` moves — so the
     re-render costs the model nothing and the printed sheet cannot notice.

     THE CURSOR IS PUT BACK ON THE BUTTON THAT WAS PRESSED. The page is rebuilt
     in one DOM write, which destroys the element holding focus; claiming the
     next focus target the way the Tab route already does returns the keyboard to
     where the reader left it, and the browser scrolls that button back into
     view — which is also what stops an opened block from throwing the page. */
  app.querySelectorAll('button[data-mtoggle]').forEach(el =>
    el.addEventListener('click', () => {
      const key = el.dataset.mtoggle;
      toggleMethodBlock(key);
      pendingFocus = {attr: 'data-mtoggle', key: key};
      reRender();
    }));
}
