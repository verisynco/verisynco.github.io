...
const V21_RENDER_VERSION = '3.80';  /* W-187: the sample toolbar's scenario
   ...
   at. No engine output moved, no hash lock moved.
   Was 3.70 for W-167: a new pure builder, introStrip() — the screen-only
   introduction band drawn above the auto-loaded sample. It is emitted by app.js
   outside both .page sheets and carries `screen-only`, so no printed sheet
   reaches it; renderReport() never calls it. The version-literal locks
   (N43/N56/N57/N60/N61/N63/N64/N65) exist so any render.js edit is a deliberate
   bump (SCHEMA § 5.1.1).
   Was 3.69 for W-162: the masthead lockup product string
...
/* W-164. THE NODE SIDE READS THE SHARED CORE, NOT A SIBLING OF THIS FILE.
   V2.1 copies the presentation layer only; the staging engine and every evidence
   record stay in v2/ and are loaded from there (v2.1/index.html says the same
   thing for the browser). `CORE` is the one place that fact is written on this
   side, so a future move of the core is one edit, not thirteen. The browser
   branch below is untouched: under file:// these modules are already globals,
   put there by the <script src="../v2/js/..."> tags in v2.1/index.html. */
const _RN = (typeof module !== 'undefined' && module.exports)
  ? (function () {
      const p = require('path');
      const CORE = p.join(__dirname, '..', '..', 'v2');
      const d = require(p.join(CORE, 'js', 'domains.js'));
      const sel = require(p.join(CORE, 'js', 'selection.js'));
      const rep = require(p.join(CORE, 'js', 'report.js'));
      const ven = require(p.join(CORE, 'js', 'vendors.js'));
      const sc = require(p.join(CORE, 'js', 'scope.js'));
      const tech = require(p.join(CORE, 'data', 'techniques.data.js'));
      return {DOMAIN_OF: d.DOMAIN_OF, CONTROLLED_UNITS: d.CONTROLLED_UNITS,
              ...
              REFERENCES: require(p.join(CORE, 'data', 'references.data.js')).REFERENCES,
              ...
              CUTOFFS: require(p.join(CORE, 'data', 'cutoffs.data.js')).CUTOFFS,
...
function masthead(profile) {
  ...
  return '<div class="masthead"><div><div class="lockup">VERISYN.CO - <b>veri.Liver</b></div>' +
    /* W-174: the mark file is SHARED from v2/assets/, not copied here. A bare
       "assets/…" resolves against /v2.1/, a folder that has never held the
       image, so this line drew a broken mark on the live site and under
       file:// alike. Same rule as every ../v2/ script this page loads (W-164):
       copy the presentation, share the evidence. */
    '<img class="brand-mark" src="../v2/assets/brand-mark.png" alt="VERISYN.CO">' +
...
/* ═══════════════════════════════════════════════════════════ W-167 — THE INTRO
   STRIP. A screen-only framing band above the auto-loaded sample. The reader
   arrives here from the access door before typing anything, so the strip's first
   job is to say plainly that the report on screen is a fabricated example, and
   its second is to offer one clear way into a real report.

   PURE builder. app.js emits it once per render, OUTSIDE both .page sheets —
   renderReport() never calls it — and it carries `screen-only` besides, the same
   belt-and-braces the toolbar uses, so no printed sheet can reach it.

   ROUND 2 (developer, 2026-09-07): the strip used to carry its own "Start a real
   report" button wired to `data-action="new-report"` — which is exactly the
   toolbar's "New Report" control, so the same action appeared twice, once per
   bar. The button is gone; the sentence names "New Report" instead, so the way
   out is still pointed at without a second copy of the control. The strip now
   has ONE control of its own: Dismiss (`data-action="dismiss-intro"`), which
   sets an in-memory flag in app.js — NOT storage (Option A): a reload shows the
   strip again, exactly as every other V2.1 screen-only flag (bmiPopupOpen,
   methodOpen) behaves. */
function introStrip(view) {
  if (!view || view.mode !== 'sample' || view.introDismissed) return '';
  return '<div class="intro-strip screen-only">' +
    '<p class="is-lead">This is a sample report — every value in it is ' +
    'fabricated for demonstration. It shows what VeriLiv produces from a set ' +
    'of liver measurements. Use New Report above to start your own.</p>' +
    '<button type="button" class="is-dismiss" data-action="dismiss-intro" ' +
      'aria-label="Dismiss this introduction">Dismiss</button>' +
  '</div>';
}
...
/* W-091. The same boxed language domainGroupHtml gives the clinical sheet's
   ...
   second wrapper. */
/* W-164. THE SAME WRAPPER, NOW A COLLAPSIBLE ONE ON SCREEN.

   The audit that opened the V2.1 line measured this sheet at 2881px of a 6187px
   screen — 47% of the scroll, carried under the clinical page in every state,
   including while the reader is still typing the first measurement. Each long
   block now opens closed and is opened by its own control, so the reader meets
   the reasoning when they ask for it rather than while they work.

   THE PAPER IS PROTECTED BY THE SHAPE OF THE CHANGE, NOT BY A RUNTIME STEP.
   Every block is BUILT IN FULL whatever its state — `html` arrives here already
   complete and is placed unconditionally — and the only thing a closed state
   changes is three attributes and one word. What hides the body is a stylesheet
   rule inside `@media screen`, which `@media print` structurally cannot see (the
   guard W-131's dark palette already uses). The browser's own native disclosure
   element is deliberately not used, and neither is any print-time event hook:
   both would make a withheld reading's reasoning depend on something happening
   at print time, and this sheet is where that reasoning lives (W-030 § 0.2,
   SCHEMA § 10.3). Naming either one literally here would also trip the
   bare-word source scans this repository already keeps (W-017 / E11).

   THE CONTROL REPEATS THE HEAD IT STANDS IN FOR. A closed box with no label is
   worse than an open one — the reader has to open it to learn whether they
   wanted it. The button carries the block's own index, its own title and a count
   of what is behind it; the printed head inside says the same words, and N69
   holds the two equal rather than trusting them to stay so (W-052). On screen
   the inner head is hidden by CSS, so the words appear once; on paper the button
   is gone (`.screen-only`) and the head is the only one left. */
function mgNoteCount(html) {
  const n = (html.match(/<p[ >]/g) || []).length;
  return n + (n === 1 ? ' note' : ' notes');
}

function mgRowCount(html) {
  return (html.match(/<tr/g) || []).length - (html.match(/<thead>/g) || []).length;
}

function mgroup(html, block) {
  if (!html) return '';
  if (!block) return '<div class="mgroup">' + html + '</div>';
  const open = block.open === true;
  const id = 'mg-' + block.key;
  /* `data-bare` — THE WRAPPER THAT MUST NOT DRAW. The evidence appendix already
     brings its own box (`section.evidence` shares the `.domaingroup`/`.mgroup`
     border rule), so wrapping it in a second drawn box would put a frame inside
     a frame ON PAPER, where no control exists to justify either. The wrapper is
     still needed — it is what carries the block key and the open state — so it
     is emitted and then neutralised: no border, no fill, no padding, no margin.
     Measured, not assumed: with this flag the printed sheet's markup differs
     from V2's by this one inert element and nothing else. */
  return '<div class="mgroup" data-mblock="' + esc(block.key) + '"' +
    (block.bare ? ' data-bare="true"' : '') +
    ' data-open="' + (open ? 'true' : 'false') + '">' +
    '<button type="button" class="mg-toggle screen-only"' +
    ' data-mtoggle="' + esc(block.key) + '"' +
    ' aria-expanded="' + (open ? 'true' : 'false') + '"' +
    ' aria-controls="' + id + '">' +
      '<span class="mg-idx">' + esc(block.idx || '') + '</span>' +
      '<span class="mg-title">' + esc(block.title) + '</span>' +
      '<span class="mg-count">' + esc(block.count) + '</span>' +
      '<span class="mg-act">' + (open ? 'Hide' : 'Show') + '</span>' +
    '</button>' +
    '<div class="mg-body" id="' + id + '">' + html + '</div>' +
  '</div>';
}

function renderMethodology(model, profile, selection, versions, view) {
  const sample = !!(view && view.mode === 'sample');
  /* W-164. Which blocks the reader has opened — an EPHEMERAL SCREEN FLAG owned by
     app.js and threaded down the way `bmiPopupOpen` already is. It is deliberately
     not part of `selection`: `selection` is the clinical model, it is what the
     hash locks cover, and a reading preference has no business moving a lock. */
  const mo = (view && view.methodOpen) || {};
  const aHtml = tableB(profile, model.labs);
  const bHtml = tableC(profile, model);
  const cHtml = tableD(profile, model, selection.indication);
  const eHtml = evidenceAppendix(model.impression && model.impression.evidence);
  const bRows = mgRowCount(bHtml);
  const eItems = (eHtml.match(/<li[ >]/g) || []).length;
  return '<div class="page pagebreak" id="methodology"' + (sample ? ' inert' : '') + '>' +
    ...
    mgroup(aHtml, {key: 'a', idx: 'A', open: mo.a === true,
                   title: 'How these numbers were formed',
                   count: mgNoteCount(aHtml)}) +
    mgroup(bHtml, {key: 'b', idx: 'B', open: mo.b === true,
                   title: 'Limitations and measurement notes',
                   count: mgNoteCount(bHtml) +
                          (bRows > 0 ? ' · ' + bRows + ' rows' : '')}) +
    /* The references count is the POOL's own length, not a count of the rows this
       block happened to draw: the pool is never filtered, and the label has to say
       the same thing the block's own note says (SCHEMA § 0, § 3.6). */
    mgroup(cHtml, {key: 'refs', idx: 'C', open: mo.refs === true,
                   title: 'References',
                   count: _RN.REFERENCES.length + ' references'}) +
    /* THE CENSUS DOES NOT COLLAPSE. It is one sentence; a control that hides one
